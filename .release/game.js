...
function loadAssets(){return loadImage('atlas','assets/game-atlas.webp?v=20260801-mega1')}
...
let player={lane:0,x:0,y:0,vy:0,slide:0,hurt:0,invuln:0};
...
function reset(){distance=0;speed=19;lives=MAX_LIVES;shake=0;time=0;player={lane:0,x:0,y:0,vy:0,slide:0,hurt:0,invuln:0};obstacles=[];particles=[];spawnTimer=999;tutorialIndex=0;tutorialPaused=false;tutorialObstacle=null;tutorialDelay=.45;updateHud()}
...
function update(dt){if(!['running','tutorial'].includes(state))return;time+=dt;player.x+=(player.lane-player.x)*Math.min(1,dt*10);if(player.y>0||player.vy>0){player.vy-=20*dt;player.y=Math.max(0,player.y+player.vy*dt);if(player.y===0&&player.vy<0)player.vy=0}player.slide=Math.max(0,player.slide-dt);player.invuln=Math.max(0,player.invuln-dt);player.hurt=Math.max(0,player.hurt-dt);shake*=Math.pow(.02,dt);
...
function coverBg(srcY,alpha=1){const im=images.atlas,iw=960,ih=540,tar=w/h;let sw,sh,sx,sy;if(iw/ih>tar){sh=ih;sw=sh*tar;sx=(iw-sw)/2;sy=srcY}else{sw=iw;sh=sw/tar;sx=0;sy=srcY+(ih-sh)/2}ctx.globalAlpha=alpha;ctx.drawImage(im,sx,sy,sw,sh,0,0,w,h);ctx.globalAlpha=1}
...
function sprite(name,x,y,scale,opts={}){const r=SPRITES[name],im=images.atlas;if(!r||!im)return;let dw=r[2]*scale,dh=r[3]*scale;ctx.save();ctx.translate(x,y);if(opts.flip)ctx.scale(-1,1);if(opts.alpha!==undefined)ctx.globalAlpha=opts.alpha;if(opts.shadow)ctx.filter='drop-shadow(0 10px 10px rgba(0,0,0,.45))';ctx.drawImage(im,r[0],r[1],r[2],r[3],-dw/2,-dh,dw,dh);ctx.restore();ctx.filter='none';ctx.globalAlpha=1}
...
window.__RAPTOR_GAME__={get state(){return state},get stats(){return{distance:Math.floor(distance),lives,lane:player.lane,y:+player.y.toFixed(2),sliding:player.slide>0,tutorialIndex,obstacles:obstacles.map(o=>({type:o.type,lane:o.lane,z:+o.z.toFixed(1)}))}},start,left:()=>act('left'),right:()=>act('right'),jump:()=>act('up'),slide:()=>act('down'),pause,resume,home,forceDamage(){hit({hit:false})},forceGoal(){distance=GOAL-1},clear(){obstacles=[];spawnTimer=99},forceObstacle(type='boulder'){obstacles.push({type,lane:player.lane,z:4,hit:false,phase:0})}};
